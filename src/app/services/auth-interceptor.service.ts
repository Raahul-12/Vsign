...
export class AuthInterceptorService implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> | any {
    ...
    if (!request.headers.has('Content-Type') && !request.url.includes('Attachment') && !request.url.includes('SaveDashboardCards') && !request.url.includes('AddDocument') && !request.url.includes('AddDocH') && !request.url.includes('AddDocApp') && !request.url.includes('AddDocAtt')&& !request.url.includes('UpdateDoc')&& !request.url.includes('UpdateAttachment')&& !request.url.includes('UpdateDocument')&& !request.url.includes('GetDocumentSigned')) {
